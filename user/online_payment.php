...
$query = "SELECT plate_number, vehicle_brand, vehicle_type FROM vehicle WHERE user_id = ? AND paid IS NULL";
...
$amount = 0;
$selected_plate_number = '';
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $selected_plate_number = $_POST['plate_number'];
    $query = "SELECT vehicle_type FROM vehicle WHERE plate_number = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('s', $selected_plate_number);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if ($vehicle_type == '4_wheel') {
            $amount = 1200;
        } elseif ($vehicle_type == '2_wheel') {
            $amount = 600;
        } elseif ($vehicle_type == '3_wheel') {
            $amount = 600;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .center-message { display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #fff; padding: 20px; border-radius: 5px; z-index: 999; }
        .center-message.show, .modal-overlay.show { display: block; }
    ...
    <div class="main-container">
        ...
        <div class="main">
            <div class="payment-container">
                ...
                <h2>Select Vehicle and Make Payment</h2>
                        ...
                        <select name="plate_number" id="vehicle" required>
                            ...
                            <?php foreach ($vehicles as $vehicle) : ?>
                                <option value="<?php echo htmlspecialchars($vehicle['plate_number']); ?>"
                                    ...
                                    <?php echo htmlspecialchars($vehicle['plate_number']) . ' - ' . htmlspecialchars($vehicle['vehicle_brand']) . ' (' . htmlspecialchars($vehicle['vehicle_type']) . ')'; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                        ...
                        <form action="process_payment.php" method="POST" id="payment-form">
                            ...
                            <div class="form-group">
                                ...
                                <div class="payment-methods">
                                    <label>
                                        <input type="radio" name="payment_method" value="credit" id="credit-card" checked>
                                        Credit Card
                                    </label>
                                    <label>
                                        <input type="radio" name="payment_method" value="debit" id="debit-card">
                                        Debit Card
                                    </label>
                                </div>
                            </div>
                            
                            <button type="submit" onclick="confirmSend(event)">Pay Now</button>
                        </form>
                    </div>
                </div>
        ...
        document.getElementById('proceed-to-payment').addEventListener('click', function() {
            ...
            if (vehicle) {
                ...
                if (vehicleType === '4_wheel') {
                    amount = 1200; 
                } else if (vehicleType === '2_wheel') {
                    amount = 600; 
                } else if (vehicleType === '3_wheel') {
                    amount = 600;
                } else {
                    amount = 0;
                }
