...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['lto_registration']) && $_FILES['lto_registration']['error'] == UPLOAD_ERR_OK) {
        $lto_registration = $upload_dir . basename($_FILES['lto_registration']['name']);
        move_uploaded_file($_FILES['lto_registration']['tmp_name'], $lto_registration);
    }

    $query = "INSERT  INTO vehicle (user_id, vehicle_brand, plate_number, vehicle_type, color, lto_registration, vehicle_picture) 
              VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('issssss', $user_id, $model, $plate_numbers, $vehicle_type, $color, $lto_registration, $vehicle_picture);
    ...
    if ($stmt->num_rows > 0) {
        ...
        echo "<script>alert('Vehicle plate number is already used');</script>";
    }
            ...
            <div class="signup-container">
                ...
                <form method="POST" action="register.php" id="register-form" enctype="multipart/form-data" onsubmit="confirmSend(event)">
                    ...
                    <div class="button-container">
                        ...
                        <button type="submit" class="submit-button" id="saymit">Register</button>
                    </div>
                </form>
